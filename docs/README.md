# User Guide
Hey there! Thanks for making mikeyparts your friend! He's here to help you with anything you may need, and this guide will help you get to know dear mikeyparts better! Let's jump right in!
...
## Getting Started
1. Java 11 is required for mikeyparts to function properly, please make sure your device has Java 11 installed. If not, you may download it [here](https://www.oracle.com/sg/java/technologies/downloads/#java11)
...
5. Use the command `java -jar ip.jar` to launch mikeyparts.
![startup.png](images/startup.png)
6. When you are finished with mikeyparts's assistance, use the `bye` command to terminate the application. This will ensure that your data is saved properly for future access.

## Features 
The below sections will briefly explore some of mikeyparts's features. mikeyparts features three types of tasks: To do, Deadline, and Event. The following sections will explore each type of task as well as other features.

### Tasks
mikeyparts contains 3 different types of tasks for your convenience. The following sections will go into closer detail on how to use these tasks.
...
The `/by` portion of the command is required for mikeyparts to distinguish between the two portions of the command. 
...
The `/from` and `/to` portions of the command are required for mikeyparts to distinguish between the two portions of the command.
...
### Marking Tasks
mikeyparts is able to help you mark tasks as complete or incomplete. New tasks are by default always added as incomplete. Incorrect use of this command will give an error message.
...
### Delete
mikeyparts unfortunately is unable to edit tasks. Should you make a mistake, you may want to delete a task. You may use the command `delete <task index>` to complete this action. For example, `delete 1`. 
...
### Saving Files
When mikeyparts is first launched, mikeyparts will create a text file in the current working directory, which should be the same directory as the location of the `.jar` file that you downloaded. 

Upon receiving the command `bye`, mikeyparts will save the information in a text file (mikeyparts.txt). The file will display the contents on individual lines, in the order of task type, completion status, description and time information (if any). An example is shown below:
...
## Conclusion
Thank you for using mikeyparts, I hope you have as much fun working with him as I did working on him! If you have any further queries or features you would like to suggest, please feel free to reach out to me at [my email](jackie.zeng@u.nus.edu)!
